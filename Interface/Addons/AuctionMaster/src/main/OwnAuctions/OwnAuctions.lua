...
local function _SetBackground(self)
	log:Debug("_SetBackground")
...
local function _HideBlizzardUi(self)
	log:Debug("_HideBlizzardUi")
	self.itemTable:Show()
	_SetBackground(self)
	...
	AuctionsScrollFrame:SetScale(0.0001)
	for i=1,15 do
		local but = _G["AuctionsButton"..i]
		if (but) then
			but:SetScale(0.0001)
		end
	end
...
local function _ShowBlizzardUi(self)
	log:Debug("_ShowBlizzardUi")
	self.itemTable:Hide(true)
	_SetBackground(self)
	...
	AuctionsScrollFrame:SetScale(1)
	for i=1,15 do
		local but = _G["AuctionsButton"..i]
		if (but) then
			but:SetScale(1)
...
local function _ApplySettings(self)
	...
	if (self.db.profile.itemTableActivated) then
		_HideBlizzardUi(self)
		_ApplyExtraLarge(self)
		_SetBackground(self)
		self:Update()
	else
		_ShowBlizzardUi(self)
	end
end
...
function vendor.OwnAuctions:OnEnable()
	...
	local statusBar = CreateFrame("StatusBar", nil, AuctionFrameAuctions, "TextStatusBar")
	statusBar.obj = self
	...
	self.frame = AuctionFrameAuctions
...
function vendor.OwnAuctions:IsVisible()
	...
	return tabType == "owner" and self.db.profile.itemTableActivated 
end
