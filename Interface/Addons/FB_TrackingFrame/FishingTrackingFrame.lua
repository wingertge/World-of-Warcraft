...
local function AddTrackingHow(how, index, id, name)
   ...
   local g = getglobal("FishingTrackingFrameGraph"..how);
...
local function UpdateColorSwatch(self, id, restore)
   ...
   local swatch = getglobal(self:GetName().."Texture");
...
FB_TrackingFrame.Update = function(self, forced)
   ...
   for idx=offset,limit do
      ...
      local locButton = getglobal(lname);
      if ( locButton ) then
         local checkh = getglobal(lname.."Hourly");
         local checkw = getglobal(lname.."Weekly");
         local icon = getglobal(lname.."IconTexture");
         local swatch = getglobal(lname.."SwatchTexture");
   ...
   while ( line <= NUM_THINGIES_DISPLAYED ) do
      local locButton = getglobal("FishingTracking"..line);
      if ( locButton ) then
         locButton:Hide();
      end
      line = line + 1;
...
FB_TrackingFrame.OnEvent = function(self, event)
   ...
   FishingBuddy.OptionsFrame.HandleOptions(FB_TFConstants.TRACKING_TAB, "Interface\\Icons\\Ability_Spy", TrackingOptions);
